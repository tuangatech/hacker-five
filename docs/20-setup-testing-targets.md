...
- **Login SQLi bypass (`POST /rest/user/login`, `email: "' OR 1=1--"`)** — real (confirmed by hand: logs in as `admin@juice-sh.op` with `role:"admin"`, no password needed) and **found**, two ways: manually (LT-192, docs/follow-up.md, 2026-09-22), `./hackerfive scan -t http://localhost:3000 --detector sqli --sqli-body-path /rest/user/login --sqli-body-param email` finds `sqli-error-email-x`, a real HTTP 500 from a genuine Sequelize/SQLite driver error, no `--allow-sqli-body-fill` needed here (Juice Shop's login route runs the query before it ever checks whether a password was supplied); and fully automatically (LT-195, docs/follow-up.md, 2026-09-23), `./hackerfive agent -t http://localhost:3000 --recon-depth full --auth-token "$TOKEN_A" --other-auth-token "$TOKEN_B" --no-model --run-every-leaf` finds the same `sqli-error-email-x` with no `--sqli-body-path`/`--sqli-body-param` at all — recon now derives the route and its `email`/`password` body fields from the bundle on its own. `sqli` has a JSON-request-body counterpart to its query-parameter checks (`sqli.Detector.RunBodyFields`) — see `tests/fixtures/known-vulns/juiceshop.json`'s `juiceshop-sqli-login-bypass` entry for the full account, including a real DBMS-signature gap live verification found and fixed along the way, and three compounding recon-extraction gaps LT-195 closed (a bare-passthrough body argument, a route built from an inline literal rather than a named constant, and the extracted fields never being wired into the endpoint-building loop that produces this route's fact).
