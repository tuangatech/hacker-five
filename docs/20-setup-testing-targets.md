...
- **Login SQLi bypass (`POST /rest/user/login`, `email: "' OR 1=1--"`)** — real (confirmed by hand: logs in as `admin@juice-sh.op` with `role:"admin"`, no password needed), but **not detectable by the current `sqli` detector** — it only appends a payload to an existing URL query parameter's value (`sqli.Target{URL, Params}`), and this bug lives in a JSON request-body field. A capability gap, not a config issue; see `tests/fixtures/known-vulns/juiceshop.json`'s `juiceshop-sqli-login-bypass` entry.
