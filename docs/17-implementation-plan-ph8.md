...
- **LT-89 (Step A) — pulled forward 2026-09-08 for the crAPI demo.** `recon --openapi-spec <file|url>` (repeatable) feeds an operator-supplied spec straight into `walkOpenAPISpec`, host-rebased + scope-checked, for the common case where the doc isn't served at a guessable unauthenticated path (crAPI's is behind `401`); `plan` passes it through. Optionally `recon --auth-token` for an authenticated crawl/probe. Read-only. Extends LT-40. Detail in [follow-up.md](follow-up.md) § "Live Testing — crAPI actionable-findings prep".
- **LT-90 (Step D) — pulled forward 2026-09-08 for the crAPI demo.** The walker records whether a route declares an OpenAPI `security:` requirement (operation-level, else document default) on the `EndpointFact`; `resolveEndpointFacts` turns an auth-declared route into an `authbypass` leaf whose `ProtectedPaths` is that route, so `checkMissingAuth` fires it tokenless. Spec-only, same lineage as LT-40/LT-50/P1-1.
