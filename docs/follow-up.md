...
| **Phase 2 auth-bypass recon breadth** | ≥10 target, real count as of 2026-08-28: 2 critical (crAPI `alg:none`) + 1 low-confidence (crAPI rate-limit) + 2 medium/low (vAPI token-reuse) confirmed, never cleanly combined into one run | Every structural blocker is fixed (configurable `--login-paths`/`--logout-paths`, configurable auth-header scheme, JSON-body probe support) — what's left is recon: more `--protected-paths` per target, and vAPI's 9 other numbered API-challenge modules (only `api1` tried). vAPI's `jwt/user` module (`JustWeakTokenController`) is a specific, known lead — its `JWT::decode()` call accepts `alg: none` per its own source, so `checkJWTAlgNone` would likely catch it — but its registration endpoint 500'd in this session and wasn't debugged further (a vAPI-side issue, not confirmed as a HackerFive bug) | doc11 Step 5 |
