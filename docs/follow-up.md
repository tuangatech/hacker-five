...
- **LT-6 tail** — the native + nuclei 1:1 finding pair now also exists for weak-HSTS
  ...
  decision. **Value: low** (report-cleanliness/dedup only, not a correctness or
  security issue) **· Effort: low** (~1 skip-list line + a wiring decision, as noted).
...
- **LT-66 tail** — the D6 catch-all verdict is host-level; a bucket host whose canary
  ...
  **Value: low-medium** (suppresses an occasional fabricated-endpoint false
  positive on bucket/storage-origin hosts specifically, not a broad class)
  **· Effort: low** (the fix is already fully designed above, just not wired up).
