...
| **Phase 2 auth-bypass recon breadth** | ≥10 target, real count as of 2026-08-28: **8** — crAPI (2 critical `alg:none` + 1 low-confidence rate-limit), vAPI `api1` (2 token-reuse), vAPI `jwt/user` (2 critical `alg:none`/signature-stripped + 1 high broken-session), never cleanly combined into one run | Every structural blocker is fixed (configurable `--login-paths`/`--logout-paths`, configurable auth-header scheme, JSON-body probe support). vAPI's `jwt/user` lead is now closed (its earlier `500` was a reused-`username` unique-constraint collision, not a bug) and yielded 3 more real findings. What's left is recon breadth: more `--protected-paths` on crAPI, and vAPI's other 8 numbered API-challenge modules (`api2`-`api10`, only `api1`/`jwt` tried) — `api9/v2/user/login` is a known positive control (`throttle:5,1` in vAPI's own routes, unlike its other login routes) worth targeting via `--login-paths` to prove the rate-limit-signal check distinguishes throttled from unthrottled | doc11 Step 5 |
