...
- **Contradiction between "read/enumerate only" and two planned detectors.** [CLAUDE.md](../CLAUDE.md) states the tool never destroys target state, but the roadmap includes "Rate Limiting Bypass" (rapid credential brute force, Week 11-12) and "Business Logic Flaws" (price manipulation, payment race conditions, Week 23-24) — both can lock real accounts or create real transactions on a live target. Resolve before those weeks are built: scope them to lab targets only (crAPI/Juice Shop), or gate behind an explicit destructive-mode flag never used against live programs.
- **No technical scope-enforcement mechanism.** The Input Parser design (doc 02) has no described allow-list/scope-file validation. Recommend a mandatory `--scope` file (domains/CIDR), validated per-request before dispatch, on by default.
- **Public OOB callback services leak target info to a third party.** The planned Interactsh use (Phase 3 SSRF, Week 21-22) should default to a self-hosted instance to preserve engagement confidentiality.

### High — architecture & tool security
- **Default rate limit too aggressive as a *default*.** "150+ req/sec" is a fine engineering benchmark but most bounty/VDP programs cap allowed request rates far lower. Default to a conservative QPS (5-10) with explicit opt-in to raise it; honor `Retry-After` on 429s as a distinct code path from the general host-error-cache (doc 02 §3) which currently only tracks consecutive errors.
- **No evidence trail captured for reports.** The `Exporter` interface (doc 02 §5) has no described mechanism to persist raw request/response pairs, which HackerOne reports need as reproducible proof. Add to the `Finding` schema now — retrofitting later touches every detector.
- **Sensitive data in exported reports.** Findings will contain live tokens/PII. Redact by default in HTML/Markdown exporters, with an explicit `--include-raw-evidence` opt-out.

### Medium — detection methodology
- **IDOR response comparison needs normalization, not just hashing.** Status/size/hash/keyword comparison (doc 01 §IDOR) will false-positive on endpoints with dynamic content (timestamps, nonces, CSRF tokens). Strip/normalize known-volatile fields before hashing, or the baseline-mode false-positive rate won't hit the "<5%" target in practice on JSON APIs.
- **JWT weak-secret dictionary check is offline, not live guessing** — worth stating explicitly in the docs so a future contributor doesn't implement it as online brute force against the live server.
- **Baseline-mode account provisioning isn't addressed.** Two-account IDOR testing requires test accounts on the target; many programs have rules about test-account creation. Process gap, not a code gap — worth a line in the docs.
...
- [03-development-roadmap.md](03-development-roadmap.md) — phase/week references throughout
