...
| BFLA — delete another user's video (`DELETE`) | ❌ miss — no detector probes non-GET methods |
...
| Mass assignment; NoSQL/SQL injection in coupon/order | ❌ miss — no native detector; injection needs a corpus run (not attempted this round) |
| JWT weak-secret; OTP-endpoint rate limiting | ❌ miss — `checkJWTWeakSecret` wordlist didn't hit; `checkRateLimitSignal` only ran the login path |

**Verdict:** the pipeline autonomously produces **13 verified actionable findings (8 critical + 5 high)** plus the `/.env` secret leak, with **zero false positives**, from `recon --openapi-spec → plan → execute` — no LLM, no manual field entry. The misses are feature gaps (LT-95/LT-96 + the injection/mass-assignment/DELETE-method backlog), not regressions.
