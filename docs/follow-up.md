...
- **P1-1 Endpoint-driven resolution pass.** A `resolveEndpointFact` alongside `resolveTechFact`: ID-shaped REST paths (`/wp-json/wp/v2/pages/12`) → `idor` leaf; `/xmlrpc.php` (200), `/wp-json/wp/v2/users` → specific template leaves; `?add-to-cart=` / `/cart/` / `/checkout/` → `businesslogic` leaf; a 500 (`/shop/software/`) → verbose-error check. `registry.Resolve` must start consuming `result.Endpoints` for leaf generation, not just for the unresolved-leaf rationale suffix.
- **P1-2 Port-driven resolution pass.** naabu found `staging.andertone.com:21` and `:3306`; the only downstream use is a low-confidence `fingerprint-port` TechFact. Ports 21/23/3306/5432/6379/9200/27017 open → a "network-service exposure" leaf (anonymous-FTP, unauth-DB, open-Elasticsearch). Needs a new capability + likely a small TCP check (see "Detection Coverage — TCP" above).
- **P1-3 WordPress plugin/theme enumeration in recon.** Parse `/wp-content/plugins/<slug>/` + `?ver=` + `readme.txt`/`style.css` version probes from `ReconResult.Endpoints` into `TechFact{Name:"wp-plugin:<slug>:<ver>"}`; match against `wp-plugin`-tagged templates by slug (917 in corpus, **0 used today**). `contact-form-7`, `litespeed-cache` both have real CVEs in the synced corpus. `pkg/recon`, `pkg/registry`.
- **P1-4 nuclei loader `body_N` / `content_type_N` / `duration` DSL gaps.** ~1,966 of 9,652 templates (20%) permanently rejected at load; a large slice are WordPress-plugin SQLi/XSS (`advanced-booking-calendar-sqli`, `3d-print-lite-xss` observed dropped) — directly costing coverage on this exact target class. Partially overlaps the "Template Engine & Detection Backlog" table above; the multi-request matcher-ref (`_2`/`_3`) support is the high-value piece. Also the natural place to add an index field carrying affected-version constraints for P0-1.
- **P1-5 `techRules` coverage.** Add `woocommerce`, `contact-form-7`, cloud providers (→ `http/` `aws`/`s3`/`gcp` exposure tags). `pkg/registry/registry.go`.
