...
|---|---|---|
| Auth bypass: 10+ issues (crAPI/vAPI) | **8 real findings, itemized** (2026-08-28): crAPI — 2 critical (`alg: none` JWT bypass, independently verified with a hand-built token via `curl`), 1 low-confidence (rate-limit-signal, path-mismatch artifact); vAPI `api1` — 2 (token-reuse via the auth-header fix, `authbypass-token-reuse-api1-user-5`/`-6`); vAPI `jwt/user` (`JustWeakTokenController`) — 3 (`authbypass-jwt-alg-none-vapi-jwt-user` and `authbypass-jwt-signature-stripped-vapi-jwt-user`, both critical and independently verified by hand-forging a `role:admin` token and retrieving the module's flag directly; `authbypass-broken-session-vapi-jwt-user`, high — the module has no server-side logout at all). Not counted: vAPI's own `authbypass-no-rate-limit-login` (same path-mismatch artifact as crAPI's, no new signal). Never yet run as one single combined scan across both targets in one pass — this is a sum of separate per-target runs, not one report | **Not met**, but close — 8 of 10+, and every remaining point is recon breadth (more `--protected-paths`/`--login-paths`), not a detector-capability gap |
...
- [x] Auth bypass detector finds real issues in crAPI/vAPI — **8 of the ≥10 target**, itemized in the Success Metrics table above (crAPI: 2 critical `alg:none` + 1 low-confidence rate-limit; vAPI `api1`: 2 token-reuse; vAPI `jwt/user`: 2 critical `alg:none`/signature-stripped + 1 high broken-session), each confirmed real and individually live-verified — but never cleanly re-run as one single combined scan across both targets in one pass, so this is a sum of separate per-target runs, not one report. See "What's still open" below for the concrete next steps to close the remaining gap
...
- **Auth bypass, more breadth**: `jwt/user` (`JustWeakTokenController`) is now closed out — see the Success Metrics row above, 3 real findings including 2 independently-verified critical `alg: none` bypasses; last session's unexplained `500` was a reused-`username` unique-constraint collision, not a real bug. Still open: more `--protected-paths` on crAPI (only `/identity/api/v2/user/dashboard`/`/vehicle/vehicles` tried) and vAPI's other 8 numbered API challenge modules (only `api1` and `jwt` tried so far; `api9/v2/user/login` is a known positive control worth targeting — it genuinely has `throttle:5,1` in vAPI's own `routes/api.php`, unlike every other login route there, so it's a good way to prove `checkRateLimitSignal` correctly distinguishes throttled from unthrottled once pointed at real login paths via `--login-paths`).
