...
| XSS: 20+ issues (DVWA/Juice Shop) | **0 → 1** — the two generic templates still find 0; a new first-party `dvwa-php/xss-reflected-dvwa.yaml` targeting DVWA's real `?name=` param (with `--header` carrying the session cookie) finds 1 real, live-verified issue. No Juice-Shop equivalent — no known server-reflected XSS surface there (its real XSS is DOM-based, deferred) | **Not met** — 1 of 20+, but the structural blocker (reachability) is closed; the gap left is breadth (more params/pages), not capability |
...
Neither doc03's ≥20 XSS nor ≥10 SQLi target is met yet (1 each, DVWA only) — the *structural* blocker (reachability) is closed, what's left is breadth: more DVWA pages/params, and there's currently no known equivalent bug on Juice Shop for this technique to reach at all.
...
- **XSS/SQLi, more breadth**: only DVWA's `xss_r`/`sqli` pages at one param each are covered. DVWA also has `xss_s` (stored) and `sqli_blind` (blind) pages — both confirmed reachable (2026-08-28, return the expected login-redirect `302` unauthenticated, same gate as the two pages already templated) but not yet templated. Juice Shop still has no server-reflected equivalent found at all — may genuinely not have one reachable by this technique.
