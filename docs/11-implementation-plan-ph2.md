...
|---|---|---|
| Auth bypass: 10+ issues (crAPI/vAPI) | **16 high-confidence findings** (2026-08-28), itemized: crAPI — 2 original (`identity` `alg:none`) + 7 more via OpenAPI-spec-driven `--protected-paths` breadth (`alg:none` proven systemic across `identity`/`community`/`workshop` — all three microservices share the unpinned-algorithm gap) + 1 signature-stripped + 1 missing-auth (`shop/return_qr_code`, no auth required at all); vAPI — 2 `api1` token-reuse + 2 `jwt/user` `alg:none`/signature-stripped + 1 `jwt/user` broken-session. Independently re-confirmed outside the tool with hand-built `curl` requests on 4 separate endpoints across both targets, not just the detector's own claim. Plus 9 lower-confidence/needs-triage findings not counted toward the headline number (2 rate-limit-signal path-mismatch artifacts, 7 token-reuse "needs manual triage" — one of which, `workshop/api/management/users/all`, is a real standalone finding on manual review: a plain non-admin account gets a full user-table PII dump). Never yet run as one single combined scan across both targets in one pass — this is a sum of separate per-target runs, not one report | **Met** — 16 ≥ 10, both targets contributed real, independently-verified findings |
| XSS: 20+ issues (DVWA/Juice Shop) | **0 → 2** — the two generic templates still find 0; two first-party templates now cover DVWA's real bugs directly (`--header` carrying the session cookie): `dvwa-php/xss-reflected-dvwa.yaml` (`?name=` param, 1 finding) and `dvwa-php/xss-stored-dvwa.yaml` (guestbook `mtxMessage` field, 1 finding, added 2026-08-28 after the auth-bypass breadth pass). No Juice-Shop equivalent — no known server-reflected XSS surface there (its real XSS is DOM-based, deferred) | **Not met** — 2 of 20+, but the structural blocker (reachability) is closed; the gap left is breadth (more pages — DVWA's `xss_d` is DOM-based, out of scope; Juice Shop has none found), not capability |
| SQLi: 10+ issues (DVWA/Juice Shop) | **0 → 2** — `dvwa-php/sqli-error-dvwa.yaml` (error-based, `?id=`) plus `dvwa-php/sqli-blind-dvwa.yaml` (boolean-based blind, added 2026-08-28 after the user explicitly chose to revisit this project's own deferred-scope decision — see below), 1 real live-verified finding each. No Juice-Shop equivalent found | **Not met** — 2 of 10+, same "capability proven, breadth still open" status as XSS |
...
**Both smaller follow-ups also fixed and live-verified (2026-08-28), same session, after explicit user request:**
- **crAPI's JSON-body probe gap.** `checkRateLimitSignal` now tries both a form-encoded and a JSON body per candidate login path during discovery (`rateLimitProbeBodies`), since a form-encoded probe against a JSON-only API returned `415 Unsupported Media Type` — not a real invalid-credential rejection — which would otherwise be misread. Live-verified against crAPI: the probe now sends `Content-Type: application/json` and gets a real `400` (server-side schema validation reached) instead of a `415`. Honest residual gap, not chased further: crAPI's real login field is `email`, not this probe's generic `username`, so the `400` is a validation error, not literally "wrong password" — still real signal about whether the endpoint throttles by request volume, just not a perfectly faithful credential-rejection probe; matching every real target's exact field names is out of scope for a generic bounded probe. Covered by a new test (`TestAuthBypassRateLimitSignal_JSONOnlyLogin_FallsBackToJSON`).
- **DVWA-specific XSS/SQLi templates.** Two new first-party (not upstream) templates, `templates/nuclei-samples/dvwa-php/{xss-reflected-dvwa,sqli-error-dvwa}.yaml`, target DVWA's real `?name=`/`?id=` params directly, carrying no credential of their own — the session cookie comes from `--header` at scan time. Live-verified against real DVWA: **1 real finding each.** No Juice-Shop equivalent was written — genuinely nothing to target there with this approach (its reflective surface is JSON, correctly excluded; its real XSS is DOM-based, deferred), not an oversight. See [../templates/nuclei-samples/dvwa-php/README.md](../templates/nuclei-samples/dvwa-php/README.md).

**Auth bypass's ≥10 target is now met (2026-08-28)**, via breadth recon rather than a further code fix: pointing `--protected-paths` at crAPI's real OpenAPI spec (`~/targets/crAPI/openapi-spec/crapi-openapi-spec.json`, not guessed paths) across 8 more endpoints on top of the original 2 surfaced 9 more high-confidence findings — the `alg:none` bypass turned out to be systemic across all three crAPI microservices (`identity`/`community`/`workshop`), not isolated to `identity`. Chasing down vAPI's `jwt/user` module (`JustWeakTokenController`) — last session's unexplained `500`, actually just a reused-username DB constraint collision — added 3 more. Full itemization and independent `curl` re-verification: see the Success Metrics table above and [20-setup-testing-targets.md](20-setup-testing-targets.md)'s crAPI/vAPI sections.

Neither doc03's ≥20 XSS nor ≥10 SQLi target is met yet (2 XSS / 2 SQLi, DVWA only) — the *structural* blocker (reachability) is closed and every known DVWA page for this technique is now templated (reflected/stored XSS, error-based/boolean-blind SQLi); what's left is breadth beyond DVWA, and there's currently no known equivalent bug on Juice Shop for either technique to reach at all.
...
- [x] XSS templates measured live against DVWA/Juice Shop — **2 findings (DVWA only), short of the ≥20 target.** The two generic upstream templates still find 0; two first-party DVWA-specific templates (reflected + stored) closed the structural (reachability) blocker and found 1 real issue each. No Juice-Shop equivalent exists — no known server-reflected XSS surface there for this technique
- [x] SQLi templates measured live against DVWA/Juice Shop — **2 findings (DVWA only), short of the ≥10 target.** Error-based plus boolean-based blind, both first-party, closing out every known DVWA SQLi page. Same pattern as XSS
...
- **Auth bypass**: ≥10 target now met (16 findings) — see the Success Metrics row above. `jwt/user` (`JustWeakTokenController`) is closed out (last session's unexplained `500` was a reused-`username` unique-constraint collision, not a real bug), and crAPI's OpenAPI-spec-driven breadth pass proved the `alg:none` gap systemic across all three microservices. Genuinely closed out as a dead end, not left open: `api9/v2/user/login` looked like a good positive control for `checkRateLimitSignal` (real `throttle:5,1` middleware, unlike every other vAPI login route) but its backing `api9users` table doesn't exist in this vAPI Docker image at all — every request 500s at the DB layer regardless of request count, a vAPI-side environment gap, not something to keep chasing. Remaining, lower-value breadth still open if wanted: vAPI's other 7 numbered API challenge modules (`api2`-`api8`, `api10`; only `api1`/`jwt` tried), and `/community/api/v2/community/posts/{postId}`-shaped endpoints that need a real ID rather than a fixed path.
- **XSS, more breadth**: `xss_s` (stored) is now templated too (`dvwa-php/xss-stored-dvwa.yaml`, 1 more live-verified finding) — DVWA's remaining XSS page, `xss_d`, is DOM-based and out of scope per this doc's own Scope section. Juice Shop still has no server-reflected equivalent found at all — may genuinely not have one reachable by this technique.
- **SQLi, blind (`sqli_blind`)**: closed out (2026-08-28) — user explicitly chose to revisit the deferred-scope decision below, given error-based SQLi's 1-of-10+ yield. `dvwa-php/sqli-blind-dvwa.yaml` fires a `raw:`-based two-request boolean differential (`id=1' AND '1'='1` vs `id=1' AND '1'='2`), correlated via `body_1`/`body_2`/`status_code_1`/`status_code_2` DSL variables — real, live-verified finding. **This surfaced a genuine engine gap along the way, now fixed**: `WithHeaders`/`--header` was only wired into `tryPath`'s request builder (`executor.go`), never `tryRaw`'s — so no `raw:`-based template could carry a session cookie at all, silently. Discovered because the blind-SQLi template needs `raw:` for the two-request correlation `path:`'s try-each-independently shape can't express. Fixed in `tryRawIteration` with the same "baseline, not override" semantics as `tryPath` (a `raw:` entry's own header line wins on a literal conflict), covered by two new tests (`TestExecutorRun_WithHeaders_AppliedToRawRequest`, `TestExecutorRun_WithHeaders_RawTemplateHeaderWins`). Separately, DVWA's blind-SQLi page turned out to return a real HTTP `404` (not `200`) for the false-condition query alongside the "MISSING" body text — an app-level signal, not a routing artifact, confirmed live and reflected in the template's matcher.
