...
| SQLi: 10+ issues (DVWA/Juice Shop) | **0 → 1** — same pattern: `dvwa-php/sqli-error-dvwa.yaml` targeting DVWA's real `?id=` param finds 1 real, live-verified issue. No Juice-Shop equivalent found | **Not met** — 1 of 10+, same "capability proven, breadth still open" status as XSS |
...
Neither doc03's ≥20 XSS nor ≥10 SQLi target is met yet (2 XSS / 1 SQLi, DVWA only) — the *structural* blocker (reachability) is closed, what's left is breadth: DVWA's remaining unfilled page is `sqli_blind` (deliberately not yet templated, a scope call — see "What's still open" below), and there's currently no known equivalent bug on Juice Shop for this technique to reach at all.
...
- **SQLi, blind (`sqli_blind`)**: confirmed reachable and confirmed exploitable by hand — `id=1' AND '1'='1` vs `id=1' AND '1'='2` gives a clean, distinguishable "exists"/"MISSING" response with no timing needed, real boolean-based SQLi. **Deliberately not templated yet**, not an oversight: `follow-up.md`'s backlog explicitly deferred boolean-based blind SQLi, with a stated trigger condition — "revisit only if error-based detection's real yield against lab targets turns out too low to be useful alone." Error-based *did* yield a real finding, so whether 1-of-10+ counts as "too low" to trigger revisiting this is a scope call worth making explicitly rather than silently building new detection capability during an open-ended breadth pass.
