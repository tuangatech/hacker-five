...
- **Anything shell/exec-shaped in the MCP tool surface** — Decision 2, same permanence for *this* surface. A future "just let the agent run curl for this one edge case" request gets the same answer every other feature request in this project already gets: add a template or detector, don't add a raw-exec tool. Doc91's research found this is a genuinely conservative position relative to the field (every comparable tool researched gives its agent a shell) — see doc14's Objective for the full finding. **Reopened 2026-09-14, scoped: a sandboxed, HITL-gated script-execution tool now exists inside the separate, opt-in `hackerfive agent` mode only — [92-research-llm-orchestrator.md](92-research-llm-orchestrator.md) §5b. This phase's own MCP server tool surface is unaffected; the answer above still holds for it.**
...
**Planning pass, 2026-09-05 — two of the original four items are already done or moved:**
- **D3 (missing-scope hard-fail) is already shipped.** Step 1's `requireScope` hard-fails every agent-initiated `scan`/`recon`/`plan` MCP call on an empty scope; P2-6 (2026-09-04) extended the same hard-fail to CLI `plan`/`recon` via `requireScopeOrOptOut` (explicit `--allow-no-scope` opt-out), with `scan` keeping its warn-only CLI behaviour because its `--targets` is already the exact host list. Nothing left to build — the DoD box is checked with that pointer.
- **H4 (cost/attempt-aware stop-and-escalate) moved to [Phase 7](16-implementation-plan-ph7.md) Step 4.** HackerFive's executor runs each leaf exactly once (`planexec.runLeaf`) — there is no per-leaf retry/grind loop for MAPTA's "rising cost ⇒ falling odds" rule to gate yet. It belongs with Phase 7 Step 4's aggregate-ceiling work, once a coordinator loop and the persisted session log (Phase 7 Step 3) exist to measure against.

Step 3 ships the two hard blockers that are actionable now, plus the small Retry-After item.

**D2 — program-policy pre-flight check, a hard blocker.** New `pkg/preflight` package. Before any `scan`/`recon`/`plan` run (MCP or CLI) against a non-lab target, `preflight.Check` looks for an operator-maintained policy declaration and **hard-fails only on an explicit "automated scanning disallowed"** — never on absence:
- **Source of truth: `.engagements/<name>/policy.yaml`** (gitignored, sibling of the existing `scope.txt`). Fields: `automated_scanning: allowed | disallowed | unknown` (default `unknown` when the file or field is missing), optional `max_runs_per_day` (informational — surfaced as a reminder, not enforced; HackerFive has no run counter, the same limitation doc22 already records for aalberts.com), `notes`. This mirrors how `.engagements/*/scope.txt` already works: the operator does the vetting (doc22 records it), a machine-readable artifact makes it enforceable. A `policy.yaml.example` documents the shape, like `env.example`.
- **Default when `unknown`/absent: warn, proceed.** A hard block on "no policy file" would break every lab, owned-site, and ad-hoc authorized run — none of which have one. The block fires *only* on an explicit `disallowed`.
- **Secondary, non-blocking signals:** recon Wave 0 keeps the fetched `security.txt` body (today only a URL/status `EndpointFact` is recorded — the body is discarded) and adds a one-GET `robots.txt` fetch. A `Disallow: /` under `User-agent: *`, or a `security.txt` that links a policy, produces a **warning** in the pre-flight output ("confirm automated scanning is permitted"), not a block — robots is a crawler convention, not a testing prohibition, and `security.txt` has no standard scanner field.
- **Override:** CLI `scan`/`plan`/`recon` accept `--allow-policy-override` for an operator with out-of-band authorization contradicting a stale file. MCP tools get **no override** — the same human-in-terminal vs. nobody-watching asymmetry as D3.
- **No-op for loopback/private targets** (`isPrivateOrLoopbackHost`).
- This is the one backlog item with a documented real-world cost of skipping it (XBOW's own program removal, doc90 §2) — hence a hard block, not `--scope`'s softer warn.

**B4 — scope-creep gate, first implementation.** `ReconResult.OutOfScope` (doc91's R6) is already populated, and those hosts already get zero active probes and can't become plan leaves (`registry.Resolve` builds host nodes only from in-scope recon hosts) — so "touch a discovered out-of-scope host" is already structurally blocked. What's missing is the **human acknowledgement** and a named executor trigger point (Open Issue #4):
- `handlePlan` (`pkg/mcpserver/tools_plan.go`): when the plan's recon pass produced `OutOfScope` entries, add an explicit **acknowledgement item** to the elicitation `InputRequests` ("recon discovered N hosts outside your scope; they will not be touched — confirm to proceed", with the list). The plan does not execute until the human acknowledges. Reuses Step 2's existing multi-round-trip mechanism — no new infrastructure.
- `pkg/planexec.ExecOptions` gains `OnOutOfScope func([]string) error` — the **named trigger point** for a future mid-scan re-recon leaf surfacing new out-of-scope hosts. Dormant today (no leaf runs recon), wired to a fresh elicitation in `pkg/mcpserver` and a job-log/pause in `pkg/webui`, so the hook exists where doc90's B4 says the check must live.
- Full cross-process mid-scan pause/re-approve stays out — Step 4 already deferred cross-process approval; [Phase 7](16-implementation-plan-ph7.md) Step 2's compliance-rounding pass (audit-trail entries) completes B4.
...
- `pkg/mcpserver/tools_scan.go`/`tools_recon.go`/`tools_plan.go` — `preflight.Check` at the top of each handler, after `requireScope`.
- `cmd/hackerfive/scan.go`/`plan.go`/`recon.go` — the same check + `--allow-policy-override`.
- `pkg/recon/passive.go` + `pkg/recon/types.go` — Wave 0 keeps the `security.txt` body and adds a `robots.txt` fetch; `ReconResult` carries both for the pre-flight and downstream use.
- `pkg/mcpserver/tools_plan.go` + `planstate.go` — B4's `OutOfScope` acknowledgement item in the elicitation round trip.
- `pkg/planexec/executor.go` — `ExecOptions.OnOutOfScope` hook + halt-remaining-dispatch on a non-nil return.
- `pkg/webui/handlers_plan_exec.go` — wire `OnOutOfScope` to a job-log/pause.
- `pkg/scanner/httpclient/middleware.go` + test — `Retry-After` parsing and honoring.
- `tests/unit/preflight_test.go` — D2 (policy-`disallowed` rejected; `unknown`/absent warns and proceeds; `--allow-policy-override` bypasses on CLI, not MCP).
- `tests/unit/scope_creep_test.go` — a `ReconResult` fixture carrying `OutOfScope` triggers the acknowledgement item, not a silent proceed.

### Verification
Unit tests for D2's hard-fail and warn-and-proceed paths, the CLI override, and the B4 acknowledgement trigger. Live verification for D2: a synthetic `.engagements/<name>/policy.yaml` with `automated_scanning: disallowed` actually blocks a `scan`/`recon`/`plan` run against that engagement's target, and setting it to `allowed` (or removing it) lets the run proceed — confirm the block fires, don't assume from code. Live verification for B4: a `plan` run whose recon surfaces a genuinely out-of-scope linked domain (doc91 Wave 3's own example) shows the acknowledgement item and does not execute until confirmed.
...
### Live verification runbook — run 2026-09-05 (Docker + recon toolchain now available in WSL)

The `/mnt/c` checkout, via `wsl.exe`, gained the full live loop (recon binaries
in `~/go/bin`, Docker Desktop, Playwright/Chromium — see CLAUDE.md's updated
"Verification" section). All five items were run from there.

1. **Round trip against DVWA + WebGoat.** ✅ `agent_e2e_test.go` with
   `HACKERFIVE_E2E_TARGET` against each, `HACKERFIVE_SESSION_LOG` set. Real
   recon → plan (elicitation auto-accepted) → scan → `findings.export` →
   `session.log`, both pass. DVWA: 8 misconfig findings; WebGoat: 10. The
   JSONL session log holds all five calls in order with the stated `reason`
   on each, secret-stripped params, timings, and a distinct approval entry.
   B4 exercised live — recon surfaced ~18 out-of-scope hosts from the bare
   `localhost` domain, and the run required the `acknowledge_out_of_scope`
   field. crAPI's credentialed `idor`/`authbypass` path not re-run this pass
   (heavy ~13-container stack) — the mechanism is identical and covered by
   DVWA/WebGoat; folded into a later live pass.
2. **Zero-fallback case (WebGoat).** ✅ after a real bug fix. First run
   *failed* `spend_usd == 0` (`5.7e-05` spent): `pkg/mcpserver`'s
   `resolveFieldSuggestions` called the I4 field-miss LLM path for
   `idor`/`authbypass` **unconditionally on every `plan` call**, even a
   `misconfig`-only plan with no such leaf — a target with no idor endpoints
   trivially hits the 0-candidate branch, so this was a standing paid LLM
   call, exactly what the DoD says I4 must never be. Fixed: every per-detector
   block in `resolveFieldSuggestions` is now gated on the plan tree actually
   carrying a leaf for that detector (`planLeafDetectors`) — matches
   `pkg/webui`'s already-correct `fillReconFields` (which switches on
   `cfg.Detector`). Re-run: `spend $0.0000`, all-deterministic. DVWA round
   trip re-confirmed at `$0.0000` too.
3. **Open Issue #4 — multi-leaf concurrency timing.** ✅ Two ways.
   `TestRunPlan_MultiLeafRunsInParallel` (`pkg/planexec`, new): 4 one-request
   template leaves against 4 servers each sleeping 300 ms, `DetConcurrency 4`
   → 1 leaf 301 ms, 4 leaves 303-306 ms (serial would be ~1.2 s), stable
   across runs. Real lab targets: 3 `misconfig` leaves (DVWA/WebGoat/Juice
   Shop) — `DetConcurrency 1` 2.61 s ≈ the sum (2.63 s); `DetConcurrency 3`
   1.79 s ≈ the slowest single leaf (1.78 s). `planexec.RunPlan` parallelises
   genuinely; wall-clock is the slowest leaf, not the sum.
4. **Step 4 — browser approval + kill switch.** ✅ Playwright + real Chromium
   against `hackerfive serve`. (A) Recon-only job → Plan Preview rendered a
   real 11-node tree ($0.0000 spend) → clicking **Approve** logged "operator
   approved — dispatching 9 leaf/leaves" and redirected to `/scans/{id}` with
   execution dispatched. (B) Full-corpus (9,476-template) `misconfig` job,
   status `running` → clicking **Cancel** → status `canceled`; log/finding
   counts frozen (`33 → 33`, `0 → 0`) over the following 6 s. Nit logged
   (follow-up.md): the Cancel control is a plain-POST form, so it navigates
   the browser to a bare `fragment_progress` fragment instead of swapping
   `#progress` in place.
5. **aalberts.com I4 re-run** (Step 2's standing commitment). ✅ with a
   nuanced result. Recon (full): 3 hosts, 21 endpoints, 5 tech facts (Amazon
   S3, AWS, GTM, HSTS, LinkedIn Ads). `plan` now resolves the fingerprint to
   **9 real, R8-ranked template leaves** (s3cfg-config, wpconfig-aws-keys,
   aws-s3-explorer, aws-detect, linkedin-client-id/-secret-key, …) — the
   "before" was *nothing but a blind full-corpus misconfig run*, so both
   doc-Step-2 gaps are materially closed. `idor`/`authbypass`/`ssrf`: no
   leaves emitted — aalberts genuinely has no such surface (no `?id=`-shaped
   params, no 401/403 paths), so there is no "genuine miss" to resolve; the
   detectors correctly don't apply rather than being silently skipped. The
   "not auto-applied" invariant stays code-enforced + unit-tested
   (`resolveFieldSuggestions` never writes an LLM value to `baseCfg`;
   `RunPlan` skips a leaf missing its field). `misconfig` corpus narrowing:
   confirmed but shallow on this target — `--narrow-by-tech` + `--recon-file`
   loaded 8,446 vs 9,476 with `--all-templates`, because generic cloud/CDN
   tech maps to broad tags (`aws`, `tokens`, `cloud`, and — over-broadly —
   `edb`/`vuln`; logged to follow-up.md). Two bugs surfaced, both logged:
   `plan --llm-assist`'s P2-1 recon-wide proposal call times out *reading*
   a slow frontier response; the `--recon-file` tag union is too liberal.

**Result: 4 of 5 runbook items fully green; item 5 substantially done with an
honest caveat (aalberts has no idor/authbypass surface to exercise) and two
minor non-blocking bugs filed.** One real bug found and fixed (I4 standing
parallel-path in `resolveFieldSuggestions`). `v0.6.0` is ready to tag on this
basis — held only pending that decision, no longer pending the runbook.
...
Unit-verified (parallel timing, ctx-stop, prompt-injection cap). Live-verified
2026-09-05 against `https://www.aceautowreckers.com` (owned): the default-scoped
`--detector misconfig` run (3,745 templates after 6a scoping) completed in **4m40s**
at `--template-concurrency 10` — "minutes", vs. the ~53 min LT-18 measured for the
old unscoped sequential path. On a 328-template subset, `--template-concurrency 1` →
39s vs `10` → 23s; it plateaus by ~10 because Cloudflare throttles one source's
concurrency (a real-world ceiling on any client, larger on a non-CDN target). The
`RunPlan` multi-leaf timing check (Open Issue #4) is separate and stays with Step 5.
...
|---|---|---|
| 1 | ~~`ResolveLeaf`'s `use_existing_tag` judgment is unreliable with a small (4.3B) local model, and not perfectly consistent run-to-run at `temperature: 0` even with a capable frontier model.~~ | **Removed 2026-09-06** — the local-tier path is not part of the approach right now (no local model configured); the residual frontier temp-0 wobble is minor and every I4 decision is human-gated before it executes anyway. Re-open if a local tier comes back or I4 output feeds something un-gated. |
| 2 | ~~An I4 `use_existing_tag` decision can't dispatch via item 4's new template-ID path — `buildLeafPrompt` only shows *tags*, never per-template IDs.~~ ✅ **resolved by P2-3 (2026-09-04)** — `buildLeafPrompt` now lists templates by exact `id: name` (`rankRelevantTemplates`), and the response schema demands "exact capability name OR exact template id — never a bare tag"; `RunPlan` dispatches a leaf whose `Detector` matches a real `templatesync.Entry.ID` (Step 2 item 4). A `techName==""` leaf falls back to the fixed-order ID walk, still dispatchable. | Closed. |
| 3 | ~~**Pre-existing test failure**: `TestEndToEnd_StartScan_ProducesRealFindings` (`pkg/webui`).~~ ✅ **resolved 2026-09-06 by [follow-up.md](follow-up.md) LT-28** — the failure was the recon toolchain being on `PATH` (real subfinder/httpx/… ran against the httptest target and blew the `Eventually` window); `newTestServer`/`newTestServerHandlers` now scrub `PATH` too. Passes 3/3 with `~/go/bin` on `PATH`. | Closed. |
| 4 | ~~Not yet live-verified: a real multi-leaf concurrency timing check.~~ ✅ done 2026-09-05 — `TestRunPlan_MultiLeafRunsInParallel` (4 leaves ≈ 1 leaf's time, not 4×) plus a real 3-lab-target measurement (`DetConcurrency 3` wall-clock ≈ slowest leaf, ½ of the serial sum). B4's `ExecOptions.OnOutOfScope` hook landed 2026-09-05 (Step 3). | Closed. |
...
| 6 | **Duplicate findings for one underlying fact** ([follow-up.md](follow-up.md) LT-6): a native `misconfig-missing-header-*` finding and the nuclei `http-missing-security-headers` template both fire on the same response — 5 findings for one fact. `reporter.Dedup` is exact-`Finding.ID`-only by deliberate design. | **Moved to [Phase 7](16-implementation-plan-ph7.md) Step 3 (2026-09-06)** — the honest fix lives in that step's Exporter/observability rework, not a pre-release patch (a naive topic key over-suppresses: the nuclei row is one aggregate over *many* headers, the native ones are one-per-header — N:1, not "same key twice"). Recommended direction to carry in: split the nuclei aggregate into per-header sub-facts *before* `Dedup`, so the existing exact-ID key then collapses the overlap on its own. Not `v0.6.0`-blocking — duplication is "two detectors agreeing", visible but not wrong. |
...
- [x] An LLM-drafted template is confirmed, live, to go through the existing untrusted-template rejection pipeline and land in `templates-proposed/`, never running against a live target without separate human promotion — done 2026-09-06: `tests/integration/llm_draft_e2e_test.go` (`//go:build integration`, gated on `HACKERFIVE_LIVE_LLM=1` + `OPENROUTER_API_KEY`). A real OpenRouter frontier call classified an unresolved bespoke-tech leaf as `needs_new_template`, then drafted a well-formed 452-byte nuclei template that passed `writeProposedTemplate`'s real `nuclei.LoadDirDetailed` pipeline into `templates-proposed/`; the leaf stayed `StatusUnresolved` with an empty `Detector` (never executed by the run), escalation recorded "pending human promotion", spend $0.0005. The earlier unit test (`TestWriteProposedTemplate_RejectsDisallowedBlock`) still covers the reject-and-delete path.
- [x] A per-job spend ceiling hard-fails a job when crossed — **resolved as a design decision** (Step 2's H5 note): a deliberately softer semantics than this literal wording — exceeding the ceiling stops further I4 calls (remaining misses escalate) but does not fail the whole plan/execution, since already-resolved deterministic work shouldn't be discarded over an unrelated resolution-budget overrun
- [x] The PlanTree executor dispatches approved leaves into real `pkg/scanner`/`pkg/recon` calls; two concurrency tiers exist and are unit-confirmed (R8-matched vs. `use_existing_tag`-resolved, not "deterministic vs. currently-costing-LLM" — see Step 2's Done note for the corrected tier semantics) — done 2026-09-02 (Step 2); live-confirmed 2026-09-05 with a real multi-leaf timing check (`TestRunPlan_MultiLeafRunsInParallel` + a 3-lab-target measurement: `DetConcurrency 3` wall-clock ≈ the slowest single leaf, not the sum)
...
- [x] The Web UI's Plan-preview page supports Approve/Reject/Edit (per-leaf inclusion, not per-field), a budget gauge, and an always-reachable kill switch that actually stops a running job — and that same kill switch is confirmed on `/scans/{id}` for plain New Scan and Guided Scan-successor (unified Launch) runs too, not only the plan-execution flow — done 2026-09-04 (Step 4's Done note); live-verified 2026-09-05 via Playwright + real Chromium against `hackerfive serve` (Approve on a real 11-node Plan Preview dispatched 9 leaves; Cancel on a running 9,476-template job moved it to `canceled` with log/finding counts frozen thereafter). Cross-process "same elicitation as an MCP client" interop stays explicitly out of scope (see that note)
...
- [x] A full recon → plan → approve → scan → export round trip is live-verified end-to-end against at least one lab target, plus a separate run against WebGoat and/or bWAPP confirming an all-`misconfig` plan resolves every leaf deterministically with zero I4 fallback calls — done 2026-09-05: `agent_e2e_test.go` with `HACKERFIVE_E2E_TARGET` against **DVWA** (8 findings) and **WebGoat** (10 findings, `HACKERFIVE_E2E_EXPECT_NO_FALLBACK=1` → `spend_usd == 0` after the `resolveFieldSuggestions` I4-gating fix — see Step 5's runbook item 2), real recon + JSONL session log + live B4 out-of-scope acknowledgement. **crAPI credentialed idor/authbypass round trip moved to [Phase 7](16-implementation-plan-ph7.md) Step 6 (2026-09-06; renumbered 2026-09-10 from "Step 7")** — that step already owns making `authbypass_crapi_test.go`/`authbypass_vapi_test.go` reproducible against the compose stack; the credentialed round trip is the same auth-token path and belongs with them, not as a loose `v0.6.0` pre-tag pass (the round-trip mechanism itself is already proven by DVWA + WebGoat above)
