...
- `pkg/llmfallback/{tiers,localmodel,openrouter}.go` — the tiered fallback client and its schema-in/schema-out call, invoked per unresolved `PlanTree` leaf **and** per unresolved doc14 Step 7 field suggestion (two small, distinct output schemas — leaf mutation vs. `{suggested_value, rationale}` — through the same tiered client).
...
- `tests/unit/llmfallback_test.go` — confirms the fallback fires only for a leaf R8 left unresolved (mocked model responses), never for an already-matched leaf; confirms a drafted template is rejected at load time if it contains a disallowed block; confirms the same client, called for a doc14 Step 7 field-suggestion miss, returns the `{suggested_value, rationale}` shape and never a `draft_template`.
