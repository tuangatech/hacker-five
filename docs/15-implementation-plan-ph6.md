...
2. **⬜ not yet implemented — wire I4 into the Web UI's plan-preview, not just `registry.Resolve`.** `pkg/webui/handlers_plan.go`'s `planPreview` calls `registry.Resolve` only — the same zero-LLM path as CLI `plan` — so an R8-unresolved leaf renders unresolved with no LLM attempt, unlike the MCP `plan` tool (this step, above), which calls `llmfallback.ResolveLeaf` for exactly this case. Scoped narrowly here: run I4 and render its decision (`use_existing_tag`/`needs_new_template`/`escalate`) plus Rationale per leaf; actually *acting* on that decision from the Web UI is Step 4's job (the approval UI), not this item's. Closing this is what would let the Web UI's Logs panel show a real "falling back to LLM" line at all — today there's nothing to log because the call never happens. Still pending scheduling.
