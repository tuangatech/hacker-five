...
**Last Updated:** 2026-09-25 (inline updates through LT-197; header bumped to match). Written after the LT-168/171/172 branch (`fix/agent-orchestrator-lt168-171-172`) and a source-level read of Strix, Cyber-AutoAgent and HexStrike (files named in §2, not launch-week claims).
...
**LT-194 built and live-verified, 2026-09-23 — closes crAPI's `contact_mechanic` SSRF, item 4(a) above, for real.** The remaining half of LT-188 (a) after the body-fill mechanism: `checkBodyParamTargets`'s response check required HTTP 200 before comparing anything, but crAPI's own convention for "the fetch didn't happen" is itself non-200 (`400 "Could not connect to mechanic api."`), so a genuinely reflected fetch was invisible no matter what its body held. Generalized the existing baseline-diff mechanism (status code or body length now count, not just length under an assumed-200 baseline) and added the target's own origin to the default internal-address sweep (the RFC1918/loopback samples already there are rarely reachable from inside a real container). Live verification against crAPI's real endpoint (no body-fill flag needed — `mechanic_api` turned out to be the route's only actually-mandatory field) found a real, byte-for-byte-confirmed reflected fetch, and, in the same run, a genuine false-positive class this session's own "prove it works" discipline caught before calling it done: three scheme-based payloads (`file://`, `gopher://`, `dict://`) crashed the endpoint into a generic framework error page that also diverges from the baseline and also contains a generic `<html` marker, misread as "fetched" until confirmed by hand that all three crash bodies were identical and contained no real file/service content. Fixed by requiring a 2xx status for the generic HTML markers to count as high confidence — a marker specific enough to real service content (redis_version, etc.) still counts at any status. Full account, including a discrepancy with LT-188's own earlier "localhost:8888 unreachable" note (re-tested today: it isn't, on this run), in follow-up.md's LT-194 entry.
...
### Real-scope measurement (LT-183 (j)): nettix.com.pe, 2026-09-25

The one gap every lab round above left open — every tree measured was 6-25 leaves, so "selection is worthless" was only shown in that small-tree regime — finally has a real, owned, non-lab data point. `TestAblationRealScope` was run three times against `nettix.com.pe` (`.engagements/owned-sites`, an owned site, not a HackerOne program — the scope file itself is the authorization), bundled templates only, `--rate-limit 10` (the default; not raised the way the lab rounds raise theirs):

| Scope | Arm | Findings | Recon endpoints | Cost | Model turns | Wall |
|---|---|---|---|---|---|---|
| 7 known hosts (`nettix-demo-focus.txt`) | no-model | 11 | 308 | $0 | 0 | 247s |
| 7 known hosts | no-model+all-leaves | 97 | 308 | $0 | 0 | 473s |
| `*.nettix.com.pe` wildcard | no-model | 17 | 308 | $0 | 0 | 313s |
| `*.nettix.com.pe` wildcard | no-model+all-leaves | 122 | 309 | $0 | 0 | 525s |
| `*.nettix.com.pe` wildcard | fast-lane+model | 101 | 309 | $0.0217 | 20 | 577s |

1. **The wildcard scope didn't actually grow the tree.** 308 → 309 recon endpoints; only `www.` and `soporte.` ever produce a finding in any arm, the same two hosts the known 7-host list already names. Real dispatchable-leaf count here is on the order of 20, not the "hundreds" this run was meant to test — **the scale question LT-183 (j) was built to answer is still open**, not because the harness or the fix is inadequate, but because this particular owned target's real footprint (a mostly server-rendered WordPress + Dolibarr site) doesn't reach that scale. It would take a genuinely larger real program to settle it.
2. **The model arm did not tie `no-model+all-leaves` here — it lost, by exactly what its own budget cost it.** A finding-ID diff shows `fast-lane+model`'s 101 findings are a strict subset of `no-model+all-leaves`'s 122: zero findings unique to the model arm, 21 unique to running everything. Of those 21: 19 are `idor-heuristic-*` hits on sequential `?p=<n>` WordPress post-ID guesses (the same low-value pagination/enumeration class the smaller baseline already showed) — plausibly reasonable to skip — but **2 are `authbypass-no-rate-limit-login` on both live hosts, a real finding class, not noise**. The model ran exactly `DefaultMaxIterations` (20) turns and the endpoint-stage histogram shows exactly 2 endpoints left `3-not-dispatched` — it ran out of iteration budget before reaching those two leaves, not because it judged them low-value. This is the first case in this doc where the model doesn't just fail to add anything beyond the deterministic ceiling — its own per-run budget actively costs real recall relative to running the tree without it.
3. **This doesn't reverse the J1/J2 answer, it reinforces it from a new angle.** Nothing here is a case where evidence existed but only a model could read it (the doc's own bar for justifying J1/J2, §4) — the miss was budget management, not missing reasoning capability, and it happened on the deterministic-selection side of the pipeline (`NextAction` ordering), which is exactly what §4's crAPI/vAPI/Juice Shop rounds already found adds nothing. If anything, it argues for revisiting `DefaultMaxIterations`/`DefaultMinIterations` (both already flagged as untuned in `pkg/orchestrator`'s own doc comments) before ever considering J1/J2, since the loss here is a scheduling problem the existing loop already has, not a gap J1/J2 would fix.
4. **A separate, precision-shaped observation, filed as LT-198, not part of the J1/J2 question:** `idor-heuristic-*` firing on every sequential `?p=<n>`/`/page/<n>` WordPress post/pagination URL is a lot of near-identical low-value findings on a public, unauthenticated CMS site — worth a closer look at whether that heuristic should recognize "sequential page/post numbers on public blog content" as a denylist pattern, independent of anything in this doc.

**Updated recommendation:** J1/J2 stay deferred — this real-target run adds a fourth case (after crAPI/vAPI/Juice Shop) where the model does not outperform the deterministic ceiling, and this time it measurably underperforms it, for a budget reason unrelated to what J1/J2 would add. The tree-scale question remains genuinely open (this target wasn't big enough to test it), so "not yet justified" is the accurate framing here, not "settled." **J4 (skeptic) and J5 (coverage ledger + stop condition) still do not depend on this answer** — both operate on findings/leaves the deterministic path already produces, so they're separable and worth scheduling on their own precision/completeness merits regardless.
...
| 0d | **Real-scope ablation** (LT-183 (j), **run 2026-09-25 against nettix.com.pe — model arm underperformed `no-model+all-leaves` on real recall, see above**): `TestAblationRealScope` against a real, owned, multi-host scope instead of a lab container | Answered "does the model beat the ceiling on a real target" (no); did **not** reach the hundreds-of-leaves scale it was meant to test, since this target's real footprint tops out around 20 dispatchable leaves — that part stays open pending a bigger real program |
| 1 | **J1 + J2 + first skill packs** (IDOR/BOLA, auth/JWT), **deferred 2026-09-21 (see "A second lab"), reconfirmed on every round since: no measured miss on any lab is beyond deterministic work; a narrow J2 is the form to try first, if 0d changes the answer** — mass assignment's narrow case (self-service PUT/PATCH, LT-133) shipped as a deterministic detector instead, 2026-09-25; a skill pack for it would only add value on the harder POST-create/nested-field shapes that detector deliberately defers | Largest expected gain: turns a closed-set orderer into a hypothesis generator, and fills the leaf fields that LT-180/LT-164 show are the actual misses — *but* not worth building until 0d says selection matters |
| 2 | **J4 skeptic + negative-control re-issue** — does not depend on step 1's outcome, schedulable independently | Largest precision gain; serves the <5% FP target and "reports that survive triage" |
| 3 | **J5 ledger + state-based stop** — does not depend on step 1's outcome, schedulable independently | Replaces the iteration-floor workaround; makes clean areas distinguishable from unvisited ones |
