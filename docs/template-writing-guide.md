...
```
Populates `.nuclei-templates-cache/http/{exposed-panels,misconfiguration,technologies,vulnerabilities,cves,exposures,default-logins}/` (gitignored) — `vulnerabilities/generic` (real, generic XSS/SQLi templates) was added in Phase 2 (see [11-implementation-plan-ph2.md](11-implementation-plan-ph2.md) Steps 2-3); the rest of `vulnerabilities/` plus `cves`/`exposures`/`default-logins` were added 2026-09-03 (see [15-implementation-plan-ph6.md](15-implementation-plan-ph6.md)'s Step 2 addendum). **This roughly triples the synced corpus** — real measured count after `hackerfive templates sync` + `templates index`: 7,716 of 9,682 templates load (79.7%; the rest hit OOB/interactsh-correlated matchers and cross-request DSL identifiers this project doesn't support yet), up from ~3,500 pulled with the original 4 categories. `--tags` to scope a scan to what's actually relevant to a target's tech stack instead of firing the whole synced corpus matters more than ever now; see [21-scanning-real-targets.md](21-scanning-real-targets.md) §3 for the full workflow.
