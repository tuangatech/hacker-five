...
4. ⬜ **Live log injection + concurrency ceilings + redundant-request elimination** (Week 53) — C4, D1, H4, D5, D6
...
- `pkg/recon/` (canary/block-page helper lifted to a shared spot; blocked-probe ratio) + `pkg/registry/decisionengine.go` (`reconShowsAdminSurface` discrimination) + `pkg/scanner/engine.go` (D6 corpus short-circuit) — D6.
...
- [ ] Uniform response wall handled (D6 / [follow-up.md](follow-up.md) LT-43(2) + LT-58 + LT-59 + LT-62): a host that answers every path identically or with a known WAF block page emits one honest note and skips the rest of the corpus (override available); `reconShowsAdminSurface` keeps the `panel` floor only for a path-discriminating 401/403; recon warns + `plan` notes when a host's blocked-probe ratio ≈ 1.0
