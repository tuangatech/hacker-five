...
## Step 6: Eval Maturity + Release (Week 56) — ✅ done 2026-09-10 — `v0.7.0` tagged
...
- [x] `v0.7.0` tagged and released — 2026-09-10, after a final pre-tag live pass against `www.aalberts.com` (a real external target with a published VDP) found and fixed two more real bugs (**LT-139**: `misconfig-cors` rated a literal wildcard `Access-Control-Allow-Origin` the same "high" as an actual reflected-origin match, though browsers refuse credentialed access on a literal wildcard regardless of `Access-Control-Allow-Credentials` — down-ranked to low/medium with a corrected description; **LT-139b**: `SuggestedAction.Detail`'s `map[string]string` type discarded a wholly valid `suggest --llm-assist` response because the model's `triage_group` action naturally returned a `finding_ids` array, not a scalar — widened to `map[string]any`); `triage --llm-assist` confirmed making genuine OpenRouter calls throughout ($0.0005–$0.0001 per call)
