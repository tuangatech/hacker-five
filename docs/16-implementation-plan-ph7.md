...
3. 🟡 **Observability upgrade: live Agent tab** (Weeks 51-52) — C6 ✅, C1/C2/C3/C5 ✅ 2026-09-07 (`ph7-step3a`), C7 ⬜ (→ `ph7-step3b`)
...
Still open in Step 3: **C7** (PlanTree structural upgrade + plausibility veto, LT-44/LT-49) — split to its own `ph7-step3b` branch: it changes the shared `pkg/agenttask` data-model shape and `pkg/registry.Resolve`'s output, higher-risk than the C1/C2/C3/C5 observability cluster and better landed (and reverted, if needed) on its own.
...
## Step 3: Observability Upgrade — Live Agent Tab (Weeks 51-52) — 🟡 C1/C2/C3/C5 done 2026-09-07 (`ph7-step3a`); C6 done earlier; C7 → `ph7-step3b`
...
### Files (anticipated — C7, `ph7-step3b`)
- `pkg/agenttask/plantree.go` — vuln-class nodes, priority field, early-leaf-output → later-leaf-input seeding.
- `pkg/registry/decisionengine.go` — `Resolve` emits the grouped/prioritised shape (not a flat leaf list).
- `pkg/llmfallback/plausibility.go` (new) — the pending-leaf veto pass.
- `pkg/planexec/executor.go` — honour the priority field when dispatching.
- `tests/unit/plantree_structure_test.go`, `tests/unit/plausibility_gate_test.go`.
