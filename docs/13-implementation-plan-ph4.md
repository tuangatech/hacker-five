...
- **OOB blind check: still pending** — needs a self-hosted `interactsh-server` (or equivalent) actually brought up; the crypto/protocol correctness is proven via the unit test's fake server, but a genuine `--oob-server` round trip against a real deployment per doc13's Definition of Done hasn't been run yet.
...
- [ ] `--ssrf-param` accepts multiple values in one scan, confirmed against a target with more than one URL-accepting parameter — not yet done; only vAPI's single `url` param has been live-tested.
- [ ] SSRF detector's OOB-based blind check live-verified against a real self-hosted callback server — a genuine `--oob-server` round trip, not just unit-tested against a mock. **Partially done**: the real RSA-OAEP+AES-256-CTR protocol/crypto round trip is proven end-to-end in `tests/unit/detector_ssrf_test.go` against a fake Interactsh-protocol server; a genuine self-hosted `interactsh-server` deployment hasn't been brought up yet.
