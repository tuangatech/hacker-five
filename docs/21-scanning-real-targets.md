...
[05-hackerone-and-legal.md](05-hackerone-and-legal.md) covers *why* and *whether* (authorization, legal/ethical rules, the HackerOne workflow). [20-setup-testing-targets.md](20-setup-testing-targets.md) covers local lab targets (crAPI, DVWA, Juice Shop, vAPI) — safe to scan freely, but not real bugs against a real program. This doc is the gap between them: once you have a genuinely authorized target (a HackerOne program, or any site with a published VDP/`security.txt` safe-harbor policy — see [01-overview-and-strategy.md](01-overview-and-strategy.md)'s Distribution & scope decision), how do you actually point HackerFive at it — what to check first, how to narrow the template set to *that* target with `--tags` instead of firing everything, and how to run the scan itself.
...
HackerFive has no crawler or fingerprinting module yet (the closest planned candidate is `follow-up.md`'s Detection Coverage section, "JS static analysis" row, unscheduled). This step is manual, a few `curl` calls, and it directly feeds step 3 below — you can't pick a relevant template set without first knowing what's actually running.
...
3. **Decide scan mode against what the scope actually allows:**
   ...
   - `--detector idor`: only if the program's rules permit creating a second test account and you already have two real accounts on the target — doc05's own Medium-severity note (via `follow-up.md`'s Security & Scope Hardening section) is that many programs restrict test-account creation; this is a manual judgment call per program, not something HackerFive checks for you.
...
- [01-overview-and-strategy.md](01-overview-and-strategy.md) — the VDP/disclose.io scope decision referenced in steps 1-2 above
- [follow-up.md](follow-up.md) — the open rate-limit-default/Retry-After gaps referenced above (Security & Scope Hardening)
