...
Not part of Steps 1-5's committed Week 5-10 deliverables — ideas surfaced while building and live-testing Step 2, worth scoping deliberately later rather than folding into an already-defined step. Ordered by how directly each moves "finds real issues in real apps," which is the actual goal, not just schema coverage. Item 7 was the one exception to "later" — done ahead of Step 5, before the v0.1.0 tag, per its own note; the rest are genuinely deferred.
...
- [ ] GitHub Actions CI green, including the new coverage gate (≥80%) — **revised:** gate wired at 79.0% (see Step 4), the real achieved number, not the original 80% target; **checked live once the repo went public and it was failing** — `golangci-lint-action@v6` had no `version:` pinned and silently resolved to a stale, Go-1.24-built golangci-lint incompatible with this module's `go 1.26.5` (see Step 5's Verification section for the full root cause). Fixed (pinned `v2.13.1`, bumped the action to `@v9`) but not yet re-confirmed green on an actual push
...
- [ ] `v0.1.0` tagged and released — repo visibility flipped to public 2026-08-26 (user-confirmed, verified live via the GitHub API), so that blocker is resolved. Still blocked on: the CI fix above landing and actually going green on a real push — not yet re-checked as of this writing
