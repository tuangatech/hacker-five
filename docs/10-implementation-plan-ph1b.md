...
Not part of Steps 1-5's committed Week 5-10 deliverables — ideas surfaced while building and live-testing Step 2, worth scoping deliberately later rather than folding into an already-defined step. Ordered by how directly each moves "finds real issues in real apps," which is the actual goal, not just schema coverage — though that ordering was a design-time guess, not measured, and item 5 turning out to matter more than item 1 once actually measured (see "Post-v0.1.0 DSL/part expansion" above) is the proof. Items 5 and 7 are done — 7 ahead of Step 5, before the v0.1.0 tag; 5 after, once #1 was picked up next and the numbers said to reorder. The rest are genuinely deferred.

1. **`raw:`/`payloads:` support.** Still real: 376 of the corpus's 475 remaining rejections (see "Post-v0.1.0 DSL/part expansion" above) are `raw:`/`payloads:` templates, including upstream's own CORS-misconfiguration template — a real check this project's own built-in `misconfig` detector approximates by hand. **Revised down from "the single biggest lever," with measured evidence:** implementing this alone nets only 84 of those 376 today (the other 292 fail on DSL gaps just fixed in the pass above — that measurement is exactly what triggered fixing the DSL gaps first). It's still a small templated-request-plus-payload-substitution engine in its own right, big enough to be its own step rather than an add-on to Step 2 — real scope worth being deliberate about: most real templates use a single payload key with the default `batteringram` mode (fire once per value, same value in every placeholder); multi-key `attack:` modes (`sniper`/`pitchfork`/`clusterbomb`) show up in only ~3% of payload-using templates sampled and add real combinatorial-request-count risk against a live target (in tension with doc21's "stay conservative" rate-limit guidance) — worth scoping as an explicit v1/v2 split rather than building the general case up front.
...
- [ ] Misconfiguration detector (built-in rules) finds ≥15 issues in DVWA — **revised down, with reasoning (2026-08-26):** real combined (misconfig + templates) result is 11 (`scripts/measure-fp-rate.sh`, Step 4) — architectural ceiling for this target, not chased further; see "Closing the two open metrics" above for why padding toward 15 isn't the right move, and Future Enhancement #4 for the real (already-scoped) lever
