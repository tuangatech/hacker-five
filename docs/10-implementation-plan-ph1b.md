...
- [ ] `v0.1.0` tagged and released — every other item above is now checked or deliberately, honestly revised (DVWA's is the one number this doc chose not to chase, with reasoning, not left silently unresolved). Nothing left blocking except actually cutting the tag and GitHub release.
