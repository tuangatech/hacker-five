...
Not part of Steps 1-5's committed Week 5-10 deliverables — ideas surfaced while building and live-testing Step 2, worth scoping deliberately later rather than folding into an already-defined step. Ordered by how directly each moves "finds real issues in real apps," which is the actual goal, not just schema coverage. Item 7 is the one exception to "later": recommended before the v0.1.0 tag, not after — see its own note.
...
7. **Capture raw request/response evidence in `Finding.Evidence`, before the v0.1.0 tag.** Flagged as a High-severity gap in [follow-up.md](follow-up.md)'s senior-security-engineer review, still open: every detector and template executor today (`idor.Detector`, `misconfig.Detector`, `nuclei.Executor`, `native.Executor`) builds `Evidence` from small derived fields only — status codes, matched pattern/header names — never the raw request/response pair a HackerOne/Markdown/HTML report needs as reproducible proof (see Step 5's exporter note above). Unlike items 1-6, this shouldn't wait: follow-up.md's own warning is that retrofitting it later "touches every detector," and today that's exactly 4 call sites (`pkg/detectors/idor/detector.go`, `pkg/detectors/misconfig/detector.go`, `pkg/template/nuclei/executor.go`, `pkg/template/native/executor.go`) — Phase 2/3 add more detectors (auth bypass, XSS, SQLi, info disclosure), so the retrofit only gets more expensive the longer it waits. `Evidence` is already `map[string]string`, so no schema change is needed, just populating `request`/`response` keys (headers + body, size-capped) at each existing call site. Pair with follow-up.md's other High-severity item, redaction: whatever captures raw evidence here should also settle the default-redact-tokens-and-PII policy Week 25's exporters will need, rather than each exporter reinventing it later.
