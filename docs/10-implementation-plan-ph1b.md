...
Not part of Steps 1-5's committed Week 5-10 deliverables — ideas surfaced while building and live-testing Step 2, worth scoping deliberately later rather than folding into an already-defined step. Ordered by how directly each moves "finds real issues in real apps," which is the actual goal, not just schema coverage — though that ordering was a design-time guess, not measured, and item 5 turning out to matter more than item 1 once actually measured (see "Post-v0.1.0 DSL/part expansion" above) is the proof. Items 1, 2, 5, and 7 are done — 7 ahead of Step 5, before the v0.1.0 tag; 5 after, once #1 was picked up next and the numbers said to reorder first; 1 right after 5, its own real prerequisite; 2 right after 1, next by the doc's own original reasoning once the prerequisite work was out of the way. The rest are genuinely deferred.
