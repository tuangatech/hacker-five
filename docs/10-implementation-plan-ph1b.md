...
- [ ] `v0.1.0` tagged and released — the packaging/infra blockers are now clear: repo visibility flipped to public 2026-08-26 (verified live), and CI is confirmed green on a real push (run #25). The two `at risk` detector-count items above (DVWA misconfig, crAPI IDOR) and the unmeasured 100-target perf target remain open with real numbers recorded, not re-litigated here — whether those are acceptable to ship v0.1.0 with is a call for the user, not something this doc should silently decide either way.
