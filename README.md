...
Phase 1a (Weeks 1-4) is done: CLI, HTTP engine, and a working **IDOR detector**. Phase 1b ([docs/10-implementation-plan-ph1b.md](docs/10-implementation-plan-ph1b.md)) is also done: misconfig detector, Nuclei-compatible template parser, native YAML template engine (`--templates`), testing/validation, and packaging. `v0.1.0` is tagged and released — see [Using HackerFive](#using-hackerfive) below to get started. See [docs/09-implementation-plan-ph1a.md](docs/09-implementation-plan-ph1a.md) for Phase 1a. Phase 2 ([docs/11-implementation-plan-ph2.md](docs/11-implementation-plan-ph2.md)) is done: `--scope` allow-list enforcement, an **API auth-bypass detector**, and reflected-XSS/error-based-SQLi/comment-leak checks. `v0.2.0` is tagged and released (2026-08-28) — real, independently-confirmed findings exist, including a critical JWT `alg:none` bypass proven systemic across all three of crAPI's microservices. Of doc03's Phase 2 Success Metrics: **auth bypass is met** (16 findings ≥ the 10+ target, across crAPI and vAPI), **false-positive rate is met** (1.4%, well under <5%); XSS/SQLi shipped short (2/20+, 2/10+, DVWA only — every known DVWA page for both techniques is templated, structural reachability is closed) — a breadth ceiling accepted as this release's honest limit, not a capability gap. See doc11 Step 5 for the full numbers and reasoning. Phase 3 is planned ([docs/12-implementation-plan-ph3.md](docs/12-implementation-plan-ph3.md)): Prompt Injection, SSRF, and Business Logic Flaws.
...
- ✅ **Templates** (`--templates`, default `./templates/`) — both formats run automatically alongside whichever `--detector` is selected, additive, not an alternative:
  ...
  - **`--header 'Name: Value'`** (repeatable) — static headers applied to every template-driven request (both formats), the primary use for a session cookie a target's login flow issued outside the scan (e.g. DVWA), since template placeholders can't carry one yet. Live-verified against a real DVWA session.
- ✅ **API Auth Bypass** (`--detector authbypass`, Phase 2, live-verified against crAPI/vAPI) — missing-authentication, JWT `alg:none`/signature-stripping bypass, an **offline-only** JWT weak-secret dictionary check (never sent to the target — see `pkg/detectors/authbypass`), a bounded rate-limit-signal probe (fixed request count, one known-invalid credential, never real credential guessing), token reuse across two accounts, and broken-session (logout-then-reuse) detection. Requires `--auth-token` and `--protected-paths` (comma-separated candidate endpoints); `--other-auth-token` additionally enables the token-reuse check. `--login-paths`/`--logout-paths` override the rate-limit/broken-session checks' fixed candidate lists (real targets rarely use the generic defaults); `--auth-header-name`/`--auth-header-format` (below) apply here too, not just to IDOR.
