...
- 🚧 **Nuclei-compatible template parser** (`pkg/template/nuclei`, plus the shared `pkg/template/matcher`/`pkg/template/extractor`/`pkg/template/dsl`) — the library is built and unit-tested (loads real upstream templates, rejects `raw:`/`payloads:`-based ones and disallowed protocol blocks), but not yet wired into `hackerfive scan`, and `scripts/sync-nuclei-templates.sh` needs a real pinned commit before the live integration test can run. Native YAML engine not started. See [docs/10-implementation-plan-ph1b.md](docs/10-implementation-plan-ph1b.md) Steps 2-3. Template files (`--templates`) are accepted on the CLI but not parsed yet — the `--endpoint` flag is the stopgap for pointing IDOR at an endpoint.
