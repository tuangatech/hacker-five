...
Phase 1a (Weeks 1-4) is done: CLI, HTTP engine, and a working **IDOR detector**. Phase 1b ([docs/10-implementation-plan-ph1b.md](docs/10-implementation-plan-ph1b.md)) is also done: misconfig detector, Nuclei-compatible template parser, native YAML template engine (`--templates`), testing/validation, and packaging. `v0.1.0` is tagged and released — see [Using HackerFive](#using-hackerfive) below to get started. See [docs/09-implementation-plan-ph1a.md](docs/09-implementation-plan-ph1a.md) for Phase 1a. Phase 2 ([docs/11-implementation-plan-ph2.md](docs/11-implementation-plan-ph2.md)) is in progress: `--scope` allow-list enforcement, an **API auth-bypass detector**, and reflected-XSS/error-based-SQLi/comment-leak checks are implemented and unit-tested. Live-verified against crAPI/vAPI/DVWA/Juice Shop (2026-08-28) — real, independently-confirmed findings exist, including a critical JWT `alg:none` bypass proven systemic across all three of crAPI's microservices. Of doc03's Phase 2 Success Metrics: **auth bypass is met** (16 findings ≥ the 10+ target, across crAPI and vAPI), **false-positive rate is met** (1.4%, well under <5%); XSS/SQLi are **not yet met** (2/20+, 2/10+, DVWA only — every known DVWA page for both techniques is templated, structural reachability is closed, what's left is breadth beyond DVWA). See doc11 Step 5 for the honest numbers, root causes, and what's already been fixed vs. still open. `v0.2.0` not yet tagged.
