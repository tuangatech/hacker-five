...
- **Plan** — a deterministic capability registry resolves recon output into a `PlanTree` of candidate detector/template work grouped by vuln-class and ordered by a confidence-derived priority, zero LLM calls, every unmapped tech signal surfaced as a visible `unresolved` leaf rather than silently dropped. Under `--llm-assist`, an optional plausibility pass can demote or drop a leaf whose recon premise looks fabricated (an SPA catch-all, a CDN-brand tech fact) — it can only ever weaken the plan, never add to it.
...
**Write-capable checks (`businesslogic`)** — an agent cannot self-grant `--allow-writes` over MCP. A `scan` call with `allow_writes` set for `detector: businesslogic` triggers a second elicitation round: the human must accept **and** tick `acknowledge_writes` before any state-changing request (coupon self-mint/apply, apply-race) is sent; a declined or elicitation-incapable client runs the scan read-only with a logged note instead. `plan` behaves the same way — a per-plan `acknowledge_writes` is required whenever the resolved tree contains a businesslogic leaf and `allow_writes` was requested. The CLI's own `hackerfive scan --allow-writes` is unchanged — a typed flag is already the explicit human decision.
