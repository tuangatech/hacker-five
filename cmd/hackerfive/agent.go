...
func newAgentCmd(root *rootFlags) *cobra.Command {
	...
	cmd.Flags().DurationVar(&maxTargetDuration, "max-target-duration", 5*time.Minute, "wall-clock cap per scan.leaf dispatch (LT-79's flag, not wired into agent until LT-179): once exceeded, no new templates start for that leaf and a scan-partial-time-budget finding is recorded instead of the leaf silently consuming the rest of the run's turn/spend budget. Unlike scan's own 15m default, agent dispatches one leaf at a time inside a run-wide iteration/budget cap, so a broad corpus leaf (e.g. misconfig) left unbounded was measured eating 16-17m of a 20m run on its own (doc94/follow-up.md LT-179) — a shorter default leaves room for the other leaves in the tree. 0 disables the cap, matching scan's own convention")
