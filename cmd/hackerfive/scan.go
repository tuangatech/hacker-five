...
func newScanCmd(root *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			cfg := scanner.Config{
				...
				SQLiBodyParams:          sqliBodyParams,
				SQLiBodyFillFields:      sqliBodyFillFields,
			...
			// LT-68 (docs/follow-up.md): echo recon's live-application-surface
			// verdict so an operator running a --recon-file scan sees up front
			// whether the target is worth the corpus pass.
			if reconResult != nil && reconResult.AppSurface != nil {
				s := reconResult.AppSurface
				lead := "scan: recon verdict"
				if s.Verdict == "none" {
					lead = "scan: WARNING — recon verdict"
				}
				_, _ = fmt.Fprintf(cmd.ErrOrStderr(), "%s: live application surface: %s (%s)\n", lead, s.Verdict, s.Reason)
			}
	...
	cmd.Flags().StringVar(&detector, "detector", "", `detector to run (required): "idor", "misconfig", "authbypass", "ssrf", "sqli" (native SQL-injection — see doc18 Step 4), "mutatebfla" (mutating-method DELETE BFLA/BOLA — see LT-132, docs/follow-up.md), "businesslogic", "netservice" (unauthenticated-exposure checks against a "tcp://host:port" target, e.g. a naabu-discovered open port — see Phase 8 Step 1, docs/17-implementation-plan-ph8.md), or "tls" (passive cert/protocol/cipher handshake checks — see Phase 8 Step 2, docs/17-implementation-plan-ph8.md)`)
	...
	cmd.Flags().StringArrayVar(&sqliBodyParams, "sqli-body-param", nil, `candidate JSON request-body field name on --sqli-body-path for the sqli detector to test (repeatable)`)
	cmd.Flags().StringArrayVar(&sqliBodyFillFields, "sqli-body-fill-field", nil, `a request-body field name (besides the one under test) to fill with a placeholder value alongside --sqli-body-param, so a target that validates its other required fields before evaluating the query (e.g. a login endpoint's password) doesn't reject the request first — repeatable, only takes effect with --allow-sqli-body-fill`)
	cmd.Flags().BoolVar(&allowSQLiBodyFill, "allow-sqli-body-fill", false, "allow the sqli detector to fill --sqli-body-fill-field's other request-body fields (with a placeholder value, never real data) so a body-field SQLi payload actually reaches the query it feeds (LT-192) — an independently-scoped exception: if the injection succeeds, getting past that validation can complete a real unauthorized action (an authentication bypass), so this is never folded into --allow-writes or --allow-ssrf-body-fill. Omitted, the sqli detector's body-field check sends only the candidate field, as before")
	...
	cmd.Flags().BoolVar(&scanUniformAnyway, "scan-uniform-anyway", false, "run the full per-target template corpus even against a host that answers every request with one WAF block / catch-all page (D6). Off by default: such a host gets one honest finding and the corpus is skipped, since every template would fetch the same page. Implied by --all-templates.")
	cmd.Flags().DurationVar(&maxTargetDuration, "max-target-duration", 15*time.Minute, "wall-clock cap per target for template dispatch (LT-79); once exceeded, no new templates start for that target and a scan-partial-time-budget finding is recorded. 0 disables the cap.")
	cmd.Flags().BoolVar(&noAdaptiveThrottle, "no-adaptive-throttle", false, "disable the adaptive rate back-off (LT-74/LT-88) that halves --rate-limit — then aborts the target — on sustained upstream 429/503 or a mid-run connect-failure spike. Leave on unless you have authorised a hard hammer.")
...
func resolveTargets(value string) ([]string, error) {
	...
	if info, err := os.Stat(value); err == nil && !info.IsDir() {
		...
		for _, line := range strings.Split(string(data), "\n") {
			// Strip an inline "# comment" first (a "#" appears in no target
			// URL we accept here), then skip blank / full-line-comment
			// lines — the same convention pkg/scanner/scope.New uses, so a
			// shared scope.txt reads identically from both (follow-up.md
			// LT-80).
