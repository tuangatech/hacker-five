...
func newScanCmd(root *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// LT-17 (docs/follow-up.md): CLI parity for LT-16's tech-stack
			// template narrowing, previously Web-UI-only.
			// registry.TechStackTags is CLI/MCP-agnostic already — scan has
			// no recon step of its own, so this reads a prior standalone
			// `hackerfive recon --output <path>`'s JSON rather than running
			// recon inline.
			if narrowByTech {
				if reconFile == "" {
					return fmt.Errorf("--narrow-by-tech requires --recon-file (a prior 'hackerfive recon --output <path>' JSON result)")
				}
				...
				// A missing/unreadable index degrades to the full corpus,
				// same "missing optional input, warn and continue" posture
				// pkg/recon/plan's own template-index loading already uses —
				// never a hard failure just because narrow-by-tech can't
				// narrow anything.
				index, idxErr := loadTemplateIndex(templateIndex)
				if idxErr != nil {
					_, _ = fmt.Fprintf(cmd.ErrOrStderr(), "narrow-by-tech: could not load %s (%v) — running the full template corpus\n", templateIndex, idxErr)
				}
				narrowScanConfigByTech(cmd.ErrOrStderr(), result.TechStack, index, &cfg)
			}
	...
	cmd.Flags().StringVar(&reconFile, "recon-file", "", "path to a prior 'hackerfive recon --output <path>' JSON result — required when --narrow-by-tech is set (LT-17, docs/follow-up.md)")
	cmd.Flags().BoolVar(&narrowByTech, "narrow-by-tech", false, "narrow --tags to --recon-file's detected tech stack, via the same tech->tag matching the decision engine/Web UI's own 'narrow templates to detected tech stack' checkbox uses (LT-16/LT-17) — never overrides an explicit --tags value; degrades to the full corpus (a stderr warning, not an error) when nothing usable is found")
	cmd.Flags().StringVar(&templateIndex, "template-index", "templates/index.json", "path to the index generated by 'hackerfive templates index', used only by --narrow-by-tech")

	return cmd
}

// narrowScanConfigByTech is LT-17's CLI parity for LT-16/registry.TechStackTags
// (docs/follow-up.md): mirrors pkg/webui/handlers_launch.go's own
// narrowConfigsByTechStack, applied to a single CLI scan.Config instead of a
// Web UI job's per-detector Config list. Never overrides an already-explicit
// --tags value (an operator's own narrowing is left alone); degrades to the
// unmodified (full-corpus) cfg, with a warning to stderr rather than an
// error, whenever nothing usable is available — the same "full corpus is
// the safe fallback" posture LT-16 established.
func narrowScanConfigByTech(stderr io.Writer, techStack []recon.TechFact, index []templatesync.Entry, cfg *scanner.Config) {
	if len(cfg.Tags) > 0 {
		return
	}
	if len(techStack) == 0 {
		_, _ = fmt.Fprintln(stderr, "narrow-by-tech: --recon-file carries no actionable technology — running the full template corpus")
		return
	}
	if len(index) == 0 {
		_, _ = fmt.Fprintln(stderr, "narrow-by-tech: no template index loaded — running the full template corpus")
		return
	}
	tags := registry.TechStackTags(techStack, index)
	if len(tags) == 0 {
		_, _ = fmt.Fprintln(stderr, "narrow-by-tech: none of the detected tech stack ties to a template tag — running the full template corpus")
		return
	}
	cfg.Tags = tags
	_, _ = fmt.Fprintf(stderr, "narrow-by-tech: narrowed to %d tech-relevant tag(s): %s\n", len(tags), strings.Join(tags, ", "))
}
