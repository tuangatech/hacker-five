		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if reconFile != "" {
				...
				if headlessCrawl && d != recon.DepthFull {
					_, _ = fmt.Fprintln(cmd.ErrOrStderr(), `plan: --headless-crawl has no effect below --recon-depth full (Wave 3 doesn't run) — ignoring`)
				}
				if paramMining && d != recon.DepthFull {
					_, _ = fmt.Fprintln(cmd.ErrOrStderr(), `plan: --param-mining has no effect below --recon-depth full (Wave 3 doesn't run) — ignoring`)
				}
				opts := []recon.Option{recon.WithRateLimit(rateLimit), recon.WithConcurrency(concurrency), recon.WithCrawlDepth(crawlDepth), recon.WithHeadlessCrawl(headlessCrawl), recon.WithParamMining(paramMining), recon.WithParamMiningWordlist(paramMiningWordlist), recon.WithParamMiningRequestCap(paramMiningReqCap)}
				if s != nil {
