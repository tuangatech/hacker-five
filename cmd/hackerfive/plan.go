		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if reconFile != "" {
				...
				if contentDiscovery && d != recon.DepthFull {
					_, _ = fmt.Fprintln(cmd.ErrOrStderr(), `plan: --content-discovery has no effect below --recon-depth full (Wave 3 doesn't run) — ignoring`)
				}
				opts := []recon.Option{recon.WithRateLimit(rateLimit), recon.WithConcurrency(concurrency), recon.WithCrawlDepth(crawlDepth), recon.WithHeadlessCrawl(headlessCrawl), recon.WithParamMining(paramMining), recon.WithParamMiningWordlist(paramMiningWordlist), recon.WithParamMiningRequestCap(paramMiningReqCap), recon.WithContentDiscovery(contentDiscovery), recon.WithContentDiscoveryWordlist(contentDiscoveryWordlist)}
				if s != nil {
