...
This run is real evidence the parser, matcher/extractor engine, and executor all work correctly end-to-end against a live target — three genuine, specific findings and one correctly-empty result from the original 2026-08-24 run. All five of this batch's templates now load cleanly as of 2026-08-26 (`apache-mod-negotiation-listing.yaml`'s `raw:`/`payloads:` rejection fixed) — see `tests/unit/nuclei_dvwa_php_samples_test.go`.
