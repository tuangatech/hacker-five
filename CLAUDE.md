...
- Lab targets (crAPI/DVWA/Juice Shop/vAPI) are for validating a mechanism, not for shaping it. When writing anything an LLM actually reads at inference time — a system/user prompt, a few-shot example, an `Action.Rationale`/leaf-rationale string, a NextAction/ResolveLeaf/Suggest/TriageFindings prompt fragment — keep the wording generic to "this host/target/endpoint," never a specific lab's name, path, or param (e.g. "crAPI," "workshop/api/mechanic," "report_id"). Naming a lab target in that text either biases the model toward a pattern that only exists in the lab or reads as a confusing aside on a real program. Go doc comments and test fixtures that cite a lab target as historical/verification evidence are fine — the constraint is on strings that flow into the model's actual input.
...
- Never add code that exfiltrates data, writes/destroys target state, or targets a host outside an explicitly authorized scope — this tool only reads/enumerates (see [docs/05-hackerone-and-legal.md](docs/05-hackerone-and-legal.md)). Two flags are this tool's only explicit, independently-scoped exceptions, each skipped with a stderr warning (never silently run) when absent: `--allow-writes`, scoped to `pkg/detectors/businesslogic`'s mutating checks (coupon mint/apply, race probes); and `--auto-provision-account`, scoped to `pkg/provision`'s throwaway second-account registration (idor/authbypass's second-account checks). Any new mutating capability needs its own equally-scoped flag, never folded into either existing one.
